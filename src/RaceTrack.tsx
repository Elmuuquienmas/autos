...
export default function RaceTrack() {
  ...
  // HISTORIAL LOCAL (Se borra si recargas la página)
  ...
  const runEngine = (carA: Car, carB: Car) => {
    ...
    if (winnerIndex === 0) speedA += 0.18; else speedB += 0.18;
  ...
  const handleRoundEnd = (winnerName: string, winnerIndex: number) => {
    ...
    setTimeout(() => {
        if (newScores.p1 >= 2 || newScores.p2 >= 2) {
            ...
            setHistory(prev => [racers[0], racers[1], ...prev].slice(0, 5)); // Guardamos los ultimos 5
            setTimeout(() => { setIsRacing(false); setRacers([]); }, 8000);
        } else {
  ...
  if (!isRacing) {
    ...
    return (
      <div className="w-screen h-screen bg-black flex items-center border-y-4 border-orange-600 px-4 overflow-hidden relative font-sans">
         ...
         {/* 1. GRUPO IZQUIERDA: JUGADOR 1 + QR (Unidos) - 40% Ancho */}
                 ...
                 <>
                   ...
                   <div className="flex flex-col justify-center h-full min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <span className="text-yellow-500 font-mono text-xs font-bold">{p1.clicks || 0} CLICKS</span>
                      </div>
                   </div>
                 </>
               ) : (
         ...
         {/* 2. CENTRO: TABLA DE RESULTADOS (Espacio Sobrante) - 25% Ancho */}
         <div className="z-10 w-[25%] h-[90%] flex flex-col bg-gray-900/50 border border-gray-700 rounded-lg p-2 mr-4 overflow-hidden">
             <h3 className="text-gray-400 text-[10px] font-bold uppercase tracking-widest border-b border-gray-700 pb-1 mb-1 text-center">Últimos Competidores</h3>
             
             {history.length === 0 ? (
                 <div className="h-full flex items-center justify-center text-gray-600 text-xs italic">Sin registros aún</div>
             ) : (
                 <div className="flex flex-col gap-1 w-full">
                     {history.slice(0, 3).map((car, idx) => ( // Solo mostramos los últimos 3 para que quepan
                         <div key={idx} className="flex items-center justify-between bg-black/40 p-1 rounded px-2">
                             <div className="flex items-center gap-2 overflow-hidden">
                                 ...
                                 <span className="text-white text-xs font-bold truncate max-w-[80px]">{car.name}</span>
                             </div>
                             <div className="flex items-center gap-2">
                                <span className="text-orange-500 text-[10px] font-mono">Lvl{car.level}</span>
                                <span className="text-yellow-500 text-[10px] font-mono">{car.clicks || 0}c</span>
                             </div>
                         </div>
                     ))}
                 </div>
  ...
  return (
    <div className="relative w-screen h-screen bg-neutral-900 overflow-hidden border-y-4 border-orange-600 flex flex-col">
      ...
      {winner && !seriesWinner && <div className="absolute inset-0 bg-black/80 flex flex-col items-center justify-center z-50"><h2 className="text-white text-lg font-bold tracking-widest uppercase">Ganador Ronda {round}</h2><div className="text-orange-500 font-black text-6xl transform -rotate-2">{winner}</div></div>}
