...
export default function RaceTrack() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(waitingRef, (snapshot) => {
      ...
      if (data) {
        const carList: Car[] = Object.entries(data).map(([key, val]: any) => ({ id: key, ...val }));
        setLobbyCars(carList);
        if (carList.length >= 2) startRaceSequence(carList[0], carList[1]);
      } else {
        setLobbyCars([]);
      }
    });
  ...
  const runEngine = (carA: Car, carB: Car) => {
    ...
    let speedA = 0.05 + (clicksA * 0.0015); 
    let speedB = 0.05 + (clicksB * 0.0015);
    if (winnerIsA) speedA += 0.02; else speedB += 0.02;

    const animate = () => {
      if (!car1Ref.current || !car2Ref.current) return;
      const jitterA = (Math.random() - 0.2) * 0.05;
      const jitterB = (Math.random() - 0.2) * 0.05;
      car1Pos.current += speedA + jitterA;
      car2Pos.current += speedB + jitterB;

      const rumbleA = Math.sin(Date.now() / 50) * 1; 
      const rumbleB = Math.cos(Date.now() / 50) * 1;

      car1Ref.current.style.transform = `translate(${car1Pos.current}vw, ${rumbleA}px)`;
      car2Ref.current.style.transform = `translate(${car2Pos.current}vw, ${rumbleB}px)`;
  ...
  const finishRace = (winnerName: string, carA: Car, carB: Car, aWon: boolean) => {
    ...
    push(ref(db, 'history'), { winner: aWon ? carA : carB, loser: aWon ? carB : carA, timestamp: Date.now() });
    setTimeout(() => {
        remove(ref(db, 'active_race')).then(() => window.location.reload());
    }, 8000);
  };
  ...
  return (
    <div className="relative w-screen h-screen bg-neutral-900 overflow-hidden border-y-4 border-orange-600 font-sans">
       <div className={`absolute inset-0 opacity-30 bg-[url('https://www.transparenttextures.com/patterns/asphalt-dark.png')] ${isRacing ? 'animate-scroll-bg' : ''}`}></div>
       <style>{`
         @keyframes scroll-bg { from { background-position: 0 0; } to { background-position: -1000px 0; } }
         .animate-scroll-bg { animation: scroll-bg 5s linear infinite; }
         .exhaust-flame { position: absolute; left: -10px; bottom: 10px; width: 20px; height: 10px; background: orange; border-radius: 50%; filter: blur(2px); opacity: 0.8; animation: flicker 0.1s infinite alternate; }
         @keyframes flicker { from { transform: scale(1); opacity: 0.8; } to { transform: scale(1.2); opacity: 0.4; } }
       `}</style>
       ...
       {isRacing && activeCars.length === 2 && (
         <div className="absolute inset-0 z-10 flex flex-col animate-in fade-in duration-300">
            ...
            <div className="relative h-1/2 w-full border-b border-dashed border-gray-600 flex items-end group">
              <div ref={car1Ref} className="absolute left-0 h-[85%] w-auto will-change-transform z-10 pl-2 pb-1">
                  ...
                  <div className="absolute -top-2 left-2 bg-orange-600 text-white text-[10px] font-black px-2 py-0.5 rounded-sm skew-x-[-10deg] shadow-lg whitespace-nowrap max-w-[200px] truncate">{activeCars[0].name}</div>
              </div>
            </div>
            <div className="relative h-1/2 w-full flex items-end">
              <div ref={car2Ref} className="absolute left-0 h-[85%] w-auto will-change-transform z-10 pl-2 pb-1">
                  ...
                  <div className="absolute -top-2 left-2 bg-blue-600 text-white text-[10px] font-black px-2 py-0.5 rounded-sm skew-x-[-10deg] shadow-lg whitespace-nowrap max-w-[200px] truncate">{activeCars[1].name}</div>
              </div>
            </div>
         </div>
       )}
       ...
       {winner && isRacing && (
           <div className="absolute inset-0 bg-gradient-to-r from-orange-900/90 to-black/90 flex flex-col items-center justify-center z-50 animate-in fade-in duration-300">
               ...
               <div className="bg-white text-black font-black px-12 py-4 text-6xl transform skew-x-[-15deg] border-4 border-orange-500 shadow-[0_0_60px_rgba(255,165,0,0.6)]">{winner}</div>
